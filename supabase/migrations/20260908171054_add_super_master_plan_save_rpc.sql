create or replace function public.save_platform_plan(
  p_id uuid,
  p_name text,
  p_slug text,
  p_description text,
...
begin
  ...
  if v_slug !~ '^[a-z0-9]+(?:-[a-z0-9]+)*$' then
    raise exception 'invalid_plan_slug' using errcode = '22023';
  end if;
  if coalesce(p_monthly_price_cents, -1) < 0 then
  ...
  values(auth.uid(),case when p_id is null then 'plan_created' else 'plan_updated' end,jsonb_build_object('plan_id',v_plan.id));
  return v_plan;
end;
$$;

revoke all on function public.save_platform_plan(uuid,text,text,text,integer,integer,integer,integer,integer,boolean,integer) from public, anon;
grant execute on function public.save_platform_plan(uuid,text,text,text,integer,integer,integer,integer,integer,boolean,integer) to authenticated;
